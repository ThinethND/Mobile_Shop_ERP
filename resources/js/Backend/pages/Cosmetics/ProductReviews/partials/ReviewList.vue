<template>
  <div class="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
    ...
    <div @click="onTableClick">
      <DataTable
        id="cosmeticProductReviewsTable"
        :url="dataUrl"
        :columns="columns"
...
const dataUrl = computed(() => route('admin.cosmetics.product-reviews.reviews.data', props.productId))
...
function onTableClick(e: MouseEvent) {
  ...
  if (action === 'edit') {
    router.visit(
      route('admin.cosmetics.product-reviews.reviews.edit', {
  ...
  if (action === 'delete') {
    ...
    router.delete(
      route('admin.cosmetics.product-reviews.reviews.destroy', {
        product: props.productId,
        review: row.id,
      }),
      {
