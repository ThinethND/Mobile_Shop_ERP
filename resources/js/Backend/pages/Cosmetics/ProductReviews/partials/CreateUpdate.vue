...
function submit() {
  ...
  if (!isEdit.value) {
    form.post(route('admin.cosmetics.product-reviews.reviews.store', props.product.id), {
      forceFormData: true,
      preserveScroll: true,
    })
    return
  ...
  form
    ...
    .post(
      route('admin.cosmetics.product-reviews.reviews.update', {
  ...
  <AppLayout>
    ...
    <div class="space-y-4 p-6">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link
          :href="route('admin.cosmetics.product-reviews.reviews.index', product.id)"
