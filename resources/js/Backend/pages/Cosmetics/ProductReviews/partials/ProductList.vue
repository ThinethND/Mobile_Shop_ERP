<template>
  <div class="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
    ...
    <div @click="onTableClick">
      <DataTable
        id="cosmeticProductReviewsProductsTable"
        :url="dataUrl"
        :columns="columns"
...
const dataUrl = computed(() => route('admin.cosmetics.product-reviews.data'))
...
function onTableClick(e: MouseEvent) {
  ...
  if (action === 'add') {
    router.visit(route('admin.cosmetics.product-reviews.reviews.create', row.id))
    return
  }

  if (action === 'view') {
    router.visit(route('admin.cosmetics.product-reviews.reviews.index', row.id))
  }
}
</script>
