<template>
  <div class="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div class="font-semibold text-neutral-800">All Cosmetic Brands</div>
...
const dataUrl = computed(() => route('admin.cosmetics.brands.data'))
...
const columnDefs = [
  { targets: [1, 4, 5], render: (data: any) => data },
]

function openCreate() {
  router.visit(route('admin.cosmetics.brands.create'))
}

function openEdit(row: BrandRow) {
  router.visit(route('admin.cosmetics.brands.edit', row.id))
}
...
function onTableClick(e: MouseEvent) {
  ...
  if (action === 'delete') {
    const ok = confirm(`Delete cosmetic brand "${row.name}"? This cannot be undone.`)
    if (!ok) return

    router.delete(route('admin.cosmetics.brands.destroy', row.id), {
      preserveScroll: true,
      onSuccess: () => {
