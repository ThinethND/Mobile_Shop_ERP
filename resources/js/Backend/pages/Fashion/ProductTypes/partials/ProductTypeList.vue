<template>
  <div class="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div class="font-semibold text-neutral-800">All Fashion Product Types</div>
...
const dataUrl = computed(() => route('admin.fashion.product-types.data'))
...
const columnDefs = [{ targets: [4, 5, 6], render: (data: any) => data }]

function openCreate() {
  router.visit(route('admin.fashion.product-types.create'))
}
...
function onTableClick(e: MouseEvent) {
  ...
  if (button.dataset.action === 'edit') {
    router.visit(route('admin.fashion.product-types.edit', row.id))
    return
  }

  if (button.dataset.action === 'delete' && confirm(`Delete product type "${row.name}"?`)) {
    router.delete(route('admin.fashion.product-types.destroy', row.id), {
      preserveScroll: true,
      onSuccess: () => {
