<template>
  <div class="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div class="font-semibold text-neutral-800">All Fashion Brands</div>
...
const dataUrl = computed(() => route('admin.fashion.brands.data'))
...
function openCreate() {
  router.visit(route('admin.fashion.brands.create'))
}
...
function onTableClick(e: MouseEvent) {
  ...
  if (button.dataset.action === 'edit') {
    router.visit(route('admin.fashion.brands.edit', row.id))
    return
  }

  if (button.dataset.action === 'delete' && confirm(`Delete fashion brand "${row.name}"?`)) {
    router.delete(route('admin.fashion.brands.destroy', row.id), {
      preserveScroll: true,
      onSuccess: () => {
