...
const props = defineProps<{
  mode: 'create' | 'edit'
  sizeType?: SizeTypePayload | null
}>()

const isEdit = computed(() => props.mode === 'edit' && !!props.sizeType?.id)

const form = useForm({
  code: props.sizeType?.code ?? '',
  name: props.sizeType?.name ?? '',
  is_active: (props.sizeType?.status ?? 'active') === 'active',
})
...
function submit() {
  ...
  if (!isEdit.value) {
    form
      ...
      .post(route('admin.shoes.size-types.store'), {
        preserveScroll: true,
  ...
  form
    ...
    .post(route('admin.shoes.size-types.update', props.sizeType!.id), {
      preserveScroll: true,
      onFinish: () => form.transform((data) => data),
    })
}
...
<template>
  <AppLayout>
    <Head :title="isEdit ? 'Update Shoe Size Type' : 'Create Shoe Size Type'" />

    <div class="space-y-4 p-6">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl font-bold">{{ isEdit ? 'Update Shoe Size Type' : 'Create Shoe Size Type' }}</h1>
          <p class="text-sm text-neutral-500">
            {{ isEdit ? 'Edit shoe size type details.' : 'Create a new shoe size type.' }}
          </p>
        </div>
