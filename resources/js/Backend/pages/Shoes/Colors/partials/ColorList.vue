<template>
  <div class="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div class="font-semibold text-neutral-800">All Shoe Colors</div>
...
const dataUrl = computed(() => route('admin.shoes.colors.data'))
const reloadKey = ref<number>(0)

const columns = [
  { data: 'id', name: 'id' },
  { data: 'name', name: 'name' },
  { data: 'hex_preview', name: 'hex_code', orderable: true, searchable: true },
  { data: 'status_badge', name: 'status', orderable: true, searchable: true },
  { data: 'actions', name: 'actions', orderable: false, searchable: false },
]

const columnDefs = [
  { targets: [2, 3, 4], render: (data: any) => data },
]

function openCreate() {
  router.visit(route('admin.shoes.colors.create'))
}

function openEdit(row: ColorRow) {
  router.visit(route('admin.shoes.colors.edit', row.id))
}
...
function onTableClick(e: MouseEvent) {
  ...
  if (action === 'delete') {
    const ok = confirm(`Delete shoe color "${row.name}"? This cannot be undone.`)
    if (!ok) return

    router.delete(route('admin.shoes.colors.destroy', row.id), {
      preserveScroll: true,
      onSuccess: () => {
