...
const isEdit = computed(() => props.mode === 'edit' && !!props.type?.id)

const form = useForm({
  name: props.type?.name ?? '',
  is_active: (props.type?.status ?? 'active') === 'active',
})
...
function submit() {
  ...
  if (!isEdit.value) {
    form
      ...
      .post(route('admin.shoes.types.store'), {
        preserveScroll: true,
  ...
  form
    ...
    .post(route('admin.shoes.types.update', props.type!.id), {
      preserveScroll: true,
      onFinish: () => form.transform((data) => data),
    })
}
...
<template>
  <AppLayout>
    <Head :title="isEdit ? 'Update Shoe Type' : 'Create Shoe Type'" />

    <div class="space-y-4 p-6">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl font-bold">{{ isEdit ? 'Update Shoe Type' : 'Create Shoe Type' }}</h1>
          <p class="text-sm text-neutral-500">
            {{ isEdit ? 'Edit shoe type details.' : 'Create a new shoe type.' }}
          </p>
        </div>
