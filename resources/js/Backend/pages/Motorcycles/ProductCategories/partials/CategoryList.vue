<template>
  <div class="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div class="font-semibold text-neutral-800">All Product Categories</div>
...
const dataUrl = computed(() => route('admin.motorcycles.categories.data'))
...
function openCreate() {
  router.visit(route('admin.motorcycles.categories.create'))
}
...
function onTableClick(e: MouseEvent) {
  ...
  if (button.dataset.action === 'edit') {
    router.visit(route('admin.motorcycles.categories.edit', row.id))
    return
  }

  if (button.dataset.action === 'delete' && confirm(`Delete category "${row.name}"?`)) {
    router.delete(route('admin.motorcycles.categories.destroy', row.id), {
      preserveScroll: true,
      onSuccess: () => {
