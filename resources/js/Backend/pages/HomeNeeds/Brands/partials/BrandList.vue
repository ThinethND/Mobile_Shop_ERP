<template>
  <div class="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div class="font-semibold text-neutral-800">All Home Need Brands</div>
...
const dataUrl = computed(() => route('admin.home-needs.brands.data'))
...
const columnDefs = [{ targets: [1, 3, 4, 5], render: (data: any) => data }]

function openCreate() {
  router.visit(route('admin.home-needs.brands.create'))
}
...
function onTableClick(e: MouseEvent) {
  ...
  if (button.dataset.action === 'edit') {
    router.visit(route('admin.home-needs.brands.edit', row.id))
    return
  }

  if (button.dataset.action === 'delete' && confirm(`Delete brand "${row.name}"?`)) {
    router.delete(route('admin.home-needs.brands.destroy', row.id), {
      preserveScroll: true,
      onSuccess: () => {
