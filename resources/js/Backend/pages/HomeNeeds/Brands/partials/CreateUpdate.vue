...
function submit() {
  if (!isEdit.value) {
    form.post(route('admin.home-needs.brands.store'), {
      forceFormData: true,
      preserveScroll: true,
    })
    return
  ...
  form
    ...
    .post(route('admin.home-needs.brands.update', props.brand!.id), {
      forceFormData: true,
      preserveScroll: true,
...
<template>
  <AppLayout>
    <Head :title="isEdit ? 'Update Home Need Brand' : 'Create Home Need Brand'" />

    <div class="space-y-4 p-6">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl font-bold">{{ isEdit ? 'Update Home Need Brand' : 'Create Home Need Brand' }}</h1>
          <p class="text-sm text-neutral-500">Add brand names and logos for cleaner product selection.</p>
        </div>

        <Link :href="route('admin.home-needs.brands.index')" class="inline-flex w-full items-center justify-center rounded-full border border-neutral-200 px-4 py-2 text-sm font-medium text-neutral-700 transition hover:bg-neutral-100 sm:w-auto">
          Back
        </Link>
      </div>
      ...
      <form @submit.prevent="submit" class="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm sm:p-6">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            ...
            <input v-model="form.name" type="text" placeholder="e.g. HomePro" class="w-full rounded-xl border border-neutral-200 px-4 py-2 outline-none focus:border-[#38bdf8]" />
        ...
        <div class="mt-6 flex flex-col gap-2 sm:flex-row sm:justify-end">
          <Link :href="route('admin.home-needs.brands.index')" class="inline-flex w-full items-center justify-center rounded-full border border-neutral-200 px-5 py-2 text-sm font-medium text-neutral-700 transition hover:bg-neutral-100 sm:w-auto">Cancel</Link>
