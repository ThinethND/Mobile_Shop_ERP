...
const pageTitle = computed(() => (isEdit.value ? 'Update Home Need Product' : 'Create Home Need Product'))
...
function submit() {
  ...
  if (!isEdit.value) {
    form.transform(payloadTransform).post(route('admin.home-needs.products.store'), {
      forceFormData: true,
      preserveScroll: true,
  ...
  form
    ...
    .post(route('admin.home-needs.products.update', props.product!.id), {
      forceFormData: true,
      preserveScroll: true,
      onFinish: () => form.transform((data: any) => data),
    })
    ...
    <div class="space-y-4 p-6">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p class="text-sm text-neutral-500">A flexible form for kitchen goods, cleaning items, storage, decor, tools, and daily home essentials.</p>
        </div>

        <Link :href="route('admin.home-needs.products.index')" class="inline-flex w-full items-center justify-center rounded-full border border-neutral-200 px-4 py-2 text-sm font-medium text-neutral-700 transition hover:bg-neutral-100 sm:w-auto">
          Back
        </Link>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-4">
        <section class="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm sm:p-6">
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div class="md:col-span-2">
              ...
              <input v-model="form.name" type="text" placeholder="e.g. Non-stick Frying Pan, Storage Box, Laundry Basket" class="w-full rounded-xl border border-neutral-200 px-4 py-2 outline-none focus:border-[#38bdf8]" />
            ...
            <div>
              <label class="mb-1 block text-sm font-medium text-neutral-700">Unit / Size</label>
              <input v-model="form.unit_label" type="text" placeholder="e.g. 1 piece, 2L, 30cm, Pack of 3" class="w-full rounded-xl border border-neutral-200 px-4 py-2 outline-none focus:border-[#38bdf8]" />
              <p v-if="form.errors.unit_label" class="mt-1 text-sm text-red-600">{{ form.errors.unit_label }}</p>
            </div>
        ...
        <section class="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm sm:p-6">
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div>
              <label class="mb-1 block text-sm font-medium text-neutral-700">Stock Status</label>
        ...
        <section class="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm sm:p-6">
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              ...
              <input v-model="form.material" type="text" placeholder="e.g. Stainless steel, cotton, plastic" class="w-full rounded-xl border border-neutral-200 px-4 py-2 outline-none focus:border-[#38bdf8]" />
              <p v-if="form.errors.material" class="mt-1 text-sm text-red-600">{{ form.errors.material }}</p>
            </div>

            <div>
              <label class="mb-1 block text-sm font-medium text-neutral-700">Color / Finish</label>
              <input v-model="form.color" type="text" placeholder="e.g. Black, white, natural wood" class="w-full rounded-xl border border-neutral-200 px-4 py-2 outline-none focus:border-[#38bdf8]" />
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div class="md:col-span-2">
              ...
              <div v-if="isEdit" class="mt-2 flex items-center gap-2">
                <input id="clearHomeNeedGallery" v-model="form.clear_gallery" type="checkbox" class="h-4 w-4" />
                <label for="clearHomeNeedGallery" class="text-sm text-neutral-700">Update all gallery images (replace existing)</label>
              </div>
            </div>
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:justify-end">
          <Link :href="route('admin.home-needs.products.index')" class="inline-flex w-full items-center justify-center rounded-full border border-neutral-200 px-5 py-2 text-sm font-medium text-neutral-700 transition hover:bg-neutral-100 sm:w-auto">Cancel</Link>
