...
class CosmeticProductReviewController extends Controller
{
    public function products()
    {
        return Inertia::render('Cosmetics/ProductReviews/ReviewProductList');
    }
    ...
    public function productsData(Request $request)
    {
        ...
        $data = $rows->map(function (CosmeticProduct $product) {
            $thumbHtml = $product->main_image_url
                ? '<div class="flex items-center gap-3">
                        <img src="' . e($product->main_image_url) . '" alt="' . e($product->name) . '" class="h-12 w-12 rounded-xl border border-neutral-200 object-cover" />
                        <div>
                            <div class="font-medium text-neutral-800">' . e($product->name) . '</div>
                            <div class="text-xs text-neutral-500">' . e($product->brand?->name ?? '-') . '</div>
                        </div>
                   </div>'
                : '<div>
                        <div class="font-medium text-neutral-800">' . e($product->name) . '</div>
    ...
    public function index(CosmeticProduct $product)
    {
        $product->loadCount('reviews')->loadAvg('reviews', 'rating');

        return Inertia::render('Cosmetics/ProductReviews/index', [
            'product' => [
                'id' => $product->id,
                'name' => $product->name,
    ...
    public function reviewsData(Request $request, CosmeticProduct $product)
    {
        $draw = (int) $request->input('draw', 1);
        $start = (int) $request->input('start', 0);
        $length = (int) $request->input('length', 10);
        ...
        $baseQuery = CosmeticProductReview::query()
        ...
        $data = $rows->map(function (CosmeticProductReview $review) {
    ...
    public function create(CosmeticProduct $product)
    {
        return Inertia::render('Cosmetics/ProductReviews/partials/CreateUpdate', [
            'mode' => 'create',
            'product' => [
                'id' => $product->id,
                'name' => $product->name,
    ...
    public function edit(CosmeticProduct $product, CosmeticProductReview $review)
    {
    ...
    public function store(Request $request, CosmeticProduct $product)
    {
        ...
        return redirect()
            ->route('admin.cosmetics.product-reviews.reviews.index', $product->id)
            ->with('success', 'Review created.');
    }

    public function update(Request $request, CosmeticProduct $product, CosmeticProductReview $review)
    {
        $this->ensureReviewBelongsToProduct($product, $review);
    ...
    public function destroy(CosmeticProduct $product, CosmeticProductReview $review)
    {
        $this->ensureReviewBelongsToProduct($product, $review);
    ...
    protected function storeImages(Request $request): array
    {
        ...
        return collect($request->file('images'))
            ->map(fn ($file) => $file->store('cosmetic-product-reviews', 'public'))
            ->values()
            ->all();
    }
    ...
    protected function ensureReviewBelongsToProduct(CosmeticProduct $product, CosmeticProductReview $review): void
    {
