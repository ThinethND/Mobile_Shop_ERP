...
class HomeNeedBrandController extends Controller
{
    public function index()
    {
        return Inertia::render('HomeNeeds/Brands/index');
    }

    public function create()
    {
        return Inertia::render('HomeNeeds/Brands/partials/CreateUpdate', [
            'mode' => 'create',
            'brand' => null,
        ]);
    }

    public function edit(HomeNeedBrand $brand)
    {
        return Inertia::render('HomeNeeds/Brands/partials/CreateUpdate', [
            'mode' => 'edit',
            'brand' => [
    ...
    public function data(Request $request)
    {
        ...
        $baseQuery = HomeNeedBrand::query()->withCount('products');
        ...
        $data = $rows->map(function (HomeNeedBrand $brand) {
    ...
    public function store(Request $request)
    {
        ...
        $logoPath = $request->hasFile('logo')
            ? $request->file('logo')->store('home-needs/brands', 'public')
            : null;

        HomeNeedBrand::create([
            ...$validated,
            'logo_path' => $logoPath,
        ]);

        return redirect()
            ->route('admin.home-needs.brands.index')
            ->with('success', 'Home need brand created.');
    }

    public function update(Request $request, HomeNeedBrand $brand)
    {
        $validated = $this->validateBrand($request, $brand);

        if ($request->hasFile('logo')) {
            $this->deleteStoredFile($brand->logo_path);
            $brand->logo_path = $request->file('logo')->store('home-needs/brands', 'public');
        }
        ...
        return redirect()
            ->route('admin.home-needs.brands.index')
            ->with('success', 'Home need brand updated.');
    }

    public function destroy(HomeNeedBrand $brand)
    {
        $this->deleteStoredFile($brand->logo_path);
        $brand->delete();
        ...
        return redirect()
            ->route('admin.home-needs.brands.index')
            ->with('success', 'Home need brand deleted.');
    }
    ...
    protected function validateBrand(Request $request, ?HomeNeedBrand $brand = null): array
    {
        return $request->validate([
            'name' => ['required', 'string', 'max:255', Rule::unique('home_need_brands', 'name')->ignore($brand?->id)],
