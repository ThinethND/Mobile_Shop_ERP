...
class HomeNeedProductController extends Controller
{
    public function index()
    {
        return Inertia::render('HomeNeeds/Products/index', $this->formOptions());
    }

    public function create()
    {
        return Inertia::render('HomeNeeds/Products/partials/CreateUpdate', array_merge($this->formOptions(), [
            'mode' => 'create',
            'product' => null,
        ]));
    }

    public function edit(HomeNeedProduct $product)
    {
        $product->load(['category:id,name', 'brand:id,name', 'warrantyOption:id,name']);
    ...
    public function data(Request $request)
    {
        ...
        $baseQuery = HomeNeedProduct::query()
            ->with(['category:id,name', 'brand:id,name', 'warrantyOption:id,name'])
        ...
        if ($searchValue !== '') {
            $baseQuery->where(function ($q) use ($searchValue) {
                $q->where('name', 'like', "%{$searchValue}%")
                    ...
                    ->orWhere('unit_label', 'like', "%{$searchValue}%")
        ...
        $data = $rows->map(function (HomeNeedProduct $product) {
            ...
            $details = '<div class="text-xs text-neutral-600 space-y-1">
                    ...
                    <div><span class="font-medium text-neutral-700">Unit:</span> ' . e($product->unit_label ?: '-') . '</div>
    ...
    public function store(Request $request)
    {
        ...
        return redirect()
            ->route('admin.home-needs.products.index')
            ->with('success', 'Home need product created.');
    }

    public function update(Request $request, HomeNeedProduct $product)
    {
        $validated = $this->validateProduct($request, $product);

        if ($request->hasFile('main_image')) {
            $this->deleteStoredFile($product->main_image_path);
            $product->main_image_path = $request->file('main_image')->store('home-needs/products', 'public');
        }

        if ($request->hasFile('hover_image')) {
            $this->deleteStoredFile($product->hover_image_path);
            $product->hover_image_path = $request->file('hover_image')->store('home-needs/products/hover', 'public');
        }
        ...
        return redirect()
            ->route('admin.home-needs.products.index')
            ->with('success', 'Home need product updated.');
    }

    public function destroy(HomeNeedProduct $product)
    {
        $this->deleteStoredFile($product->main_image_path);
        $this->deleteStoredFile($product->hover_image_path);
        ...
        return redirect()
            ->route('admin.home-needs.products.index')
            ->with('success', 'Home need product deleted.');
    }

    public function toggleStatus(HomeNeedProduct $product)
    {
        $product->status = $product->status === 'active' ? 'inactive' : 'active';
        $product->save();

        return back()->with('success', 'Home need product status updated.');
    }

    protected function validateProduct(Request $request, ?HomeNeedProduct $product = null): array
    {
        return $request->validate([
            'name' => ['required', 'string', 'max:255'],
            'category_id' => ['nullable', 'integer', 'exists:home_need_categories,id'],
            'brand_id' => ['nullable', 'integer', 'exists:home_need_brands,id'],
            'brand_name' => ['nullable', 'string', 'max:255'],
            'sku' => ['nullable', 'string', 'max:255', Rule::unique('home_need_products', 'sku')->ignore($product?->id)],
    ...
    {
        return [
            'categories' => HomeNeedCategory::query()
                ->where('status', 'active')
                ->orderBy('name')
                ->get(['id', 'name'])
                ...
                ->values(),
            'brands' => HomeNeedBrand::query()
                ->where('status', 'active')
                ->orderBy('name')
                ->get(['id', 'name'])
                ->map(fn (HomeNeedBrand $brand) => [
                    'id' => $brand->id,
                    'name' => $brand->name,
                    'value' => $brand->id,
                    'label' => $brand->name,
    ...
    {
        $base = Str::slug($name) ?: 'home-need-product';
        $slug = $base;
        $index = 2;

        while (HomeNeedProduct::query()
            ->where('slug', $slug)
    ...
    protected function storeGalleryImages(Request $request): array
    {
        ...
        return collect($request->file('gallery_images', []))
            ->map(fn ($file) => $file->store('home-needs/products/gallery', 'public'))
            ->values()
            ->all();
    }

    protected function warrantyDisplay(HomeNeedProduct $product): string
    {
    ...
    protected function brandDisplay(HomeNeedProduct $product): string
    {
