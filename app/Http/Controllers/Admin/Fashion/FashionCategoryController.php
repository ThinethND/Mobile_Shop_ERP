...
class FashionCategoryController extends Controller
{
    ...
    {
        return Inertia::render('Fashion/Categories/index');
    }

    public function create()
    {
        return Inertia::render('Fashion/Categories/partials/CreateUpdate', [
            'mode' => 'create',
            'category' => null,
        ]);
    }

    public function edit(FashionCategory $category)
    {
        return Inertia::render('Fashion/Categories/partials/CreateUpdate', [
            'mode' => 'edit',
            'category' => [
    ...
    public function data(Request $request)
    {
        ...
        $baseQuery = FashionCategory::query()->withCount('products');
        ...
        $data = $rows->map(function (FashionCategory $category) {
    ...
    {
        FashionCategory::create($this->validateCategory($request));

        return redirect()
            ->route('admin.fashion.categories.index')
            ->with('success', 'Fashion category created.');
    }

    public function update(Request $request, FashionCategory $category)
    {
        $category->update($this->validateCategory($request, $category));

        return redirect()
            ->route('admin.fashion.categories.index')
            ->with('success', 'Fashion category updated.');
    }

    public function destroy(FashionCategory $category)
    {
        $category->delete();

        return redirect()
            ->route('admin.fashion.categories.index')
            ->with('success', 'Fashion category deleted.');
    }
    ...
    protected function validateCategory(Request $request, ?FashionCategory $category = null): array
    {
        return $request->validate([
            'name' => ['required', 'string', 'max:255', Rule::unique('fashion_categories', 'name')->ignore($category?->id)],
