...
class ShoeProductReviewController extends Controller
{
    public function products()
    {
        return Inertia::render('Shoes/ProductReviews/ReviewProductList');
    }
    ...
    public function index(ShoeProduct $product)
    {
        $product->loadCount('reviews')->loadAvg('reviews', 'rating');

        return Inertia::render('Shoes/ProductReviews/index', [
            'product' => [
                'id' => $product->id,
                'name' => $product->name,
    ...
    public function reviewsData(Request $request, ShoeProduct $product)
    {
        $draw = (int) $request->input('draw', 1);
        $start = (int) $request->input('start', 0);
        $length = (int) $request->input('length', 10);
        ...
        $baseQuery = ShoeProductReview::query()
        ...
        $data = $rows->map(function (ShoeProductReview $review) {
    ...
    public function create(ShoeProduct $product)
    {
        return Inertia::render('Shoes/ProductReviews/partials/CreateUpdate', [
            'mode' => 'create',
            'product' => [
                'id' => $product->id,
                'name' => $product->name,
    ...
    public function edit(ShoeProduct $product, ShoeProductReview $review)
    {
    ...
    public function store(Request $request, ShoeProduct $product)
    {
        ...
        return redirect()
            ->route('admin.shoes.product-reviews.reviews.index', $product->id)
            ->with('success', 'Review created.');
    }

    public function update(Request $request, ShoeProduct $product, ShoeProductReview $review)
    {
        $this->ensureReviewBelongsToProduct($product, $review);
    ...
    public function destroy(ShoeProduct $product, ShoeProductReview $review)
    {
        $this->ensureReviewBelongsToProduct($product, $review);
    ...
    protected function ensureReviewBelongsToProduct(ShoeProduct $product, ShoeProductReview $review): void
    {
